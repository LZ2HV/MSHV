...
int PomFt::SyncQualFt2_4(double *bitmetrics)
{
    ...
    const bool ms1[8] = {0,0,0,1,1,0,1,1};//count(hbits(  1:  8)==(/0,0,0,1,1,0,1,1/))
    int ns1=count_eq_bits(hbits,0,  ms1,8);
    const bool ms2[8] = {0,1,0,0,1,1,1,0};//count(hbits( 67: 74)==(/0,1,0,0,1,1,1,0/))
    int ns2=count_eq_bits(hbits,66, ms2,8);
    const bool ms3[8] = {1,1,1,0,0,1,0,0};//count(hbits(133:140)==(/1,1,1,0,0,1,0,0/))
    int ns3=count_eq_bits(hbits,132,ms3,8);
    const bool ms4[8] = {1,0,1,1,0,0,0,1};//count(hbits(199:206)==(/1,0,1,1,0,0,0,1/))
