...
#define Q_PI 3.14159265358
#define BEGIN_STYLE_PIXMAPCACHE(a) \
    QRect rect = option->rect; \
    QPixmap internalPixmapCache; \
    QImage imageCache; \
    QPainter *p = painter; \
    QString unique = QStyleHelper1::uniqueName((a), option, option->rect.size()); \
...
static QPointF calcRadialPos(const QStyleOptionSlider *dial, qreal offset)
{
    ...
    qreal xc = width / 2.0;
    qreal yc = height / 2.0;
    qreal len = r - QStyleHelper1::calcBigLineSize(r) - 3;
...
QPolygonF calcLines(const QStyleOptionSlider *dial)
{
    ...
    {
        qreal angle1 = dial->dialWrapping ? Q_PI * 3 / 2 - i * 2 * Q_PI / notches
...
void drawDial(const QStyleOptionSlider *option, QPainter *painter)
{
    ...
    if (option->subControls & QStyle::SC_DialTickmarks)
    {
        ...
        painter->drawLines(QStyleHelper1::calcLines(option));
    }
...
HvStylePlastique::HvStylePlastique(bool f)//:
//: QCommonStyle(*new HvStylePlastiquePrivate)
//: HvPublicStyle(QStyleFactory::create("windows"))//,(*new HvStylePlastiquePrivate)
{
    f_dark_style = f;
    ...
    //int nFont = QFontDatabase::addApplicationFont(":font/arial.ttf");//registriram fonta//  xsuni
    ...
    //int nFont = QFontDatabase::addApplicationFont (App_Path+"/settings/font/cour.ttf");
    ...
    #endif
    #if defined _LINUX_
        QFont font(szFont, 10);
    #endif
    */
    //QFont font(fontList[0],9);
    //QApplication::setFont(font);
    //QApplication::setStyleSheet("color: black; font-family: Arial; font-size: 14px");
    //QApplication:: painter.setFont(font);
    // painter->setFont(font);


}
HvStylePlastique::~HvStylePlastique()
{}
int HvStylePlastique::pixelMetric(PixelMetric pm, const QStyleOption *opt, const QWidget *widget) const
{
    switch (pm)
    {
    case  PM_LayoutVerticalSpacing:
        return 5; // raztoianieto m/u layoutite / v id3 info
    case PM_FocusFrameVMargin:
        return 0;// na buton 6irinata ot dolu
    case PM_FocusFrameHMargin:
        return 4; //treeview otpred pred rowa
    case  PM_DefaultFrameWidth:
        return 2; // dalzinata na row v treeview i bordera
    case PM_HeaderGripMargin:
        return 5; // raztoianieto ot koeto se pokazva splitera za resize
    case PM_HeaderMargin:
        return 2;// goleminata na headera
    ...
    case PM_ScrollBarExtent:
        return 16;
    default:
        return HvPublicStyle::pixelMetric(pm, opt, widget);
    }
    ...
    /* int ret;
       ...
       case PM_SliderLength:
           ret = int(QStyleHelper1::dpiScaled(11.));
           break;
       ...
       case PM_SmallIconSize:
           ret = int(QStyleHelper1::dpiScaled(16.));
           break;

       case PM_LargeIconSize:
           ret = int(QStyleHelper1::dpiScaled(32.));
           break;
       ...
       case PM_DockWidgetTitleMargin:
           ret = int(QStyleHelper1::dpiScaled(2.));
           break;
       case PM_DockWidgetTitleBarButtonMargin:
           ret = int(QStyleHelper1::dpiScaled(4.));
           break;
    #if defined(Q_WS_WIN)
       ...
       case PM_ToolBarHandleExtent:
           ret = int(QStyleHelper1::dpiScaled(10.));
           break;
       default:
           ret = QCommonStyle::pixelMetric(pm, opt, widget);
           break;
    ...
    /*  int ret = -1;
      ...
      case PM_TitleBarHeight:
          ...
          ret = qMax(widget ? widget->fontMetrics().height() :
                     (opt ? opt->fontMetrics.height() : 0), 30);
          break;
      case PM_MaximumDragDistance:
          return -1;
      case PM_DockWidgetTitleMargin:
      ...
      return ret != -1 ? ret : HvPublicStyle::pixelMetric(pm, opt, widget);*/
}

QRect HvStylePlastique::subElementRect ( SubElement element, const QStyleOption * option, const QWidget * widget ) const
{
    /* switch (element)
       {
       case SE_ItemViewItemText:
       {
           QRect rect = HvPublicStyle::subElementRect( SE_ItemViewItemText, option, widget );
           rect.adjust( 0, 1, 0, 1 );//poziciata na text na row
           return rect;
       }
       case SE_HeaderLabel:
       {
           QRect rect = HvPublicStyle::subElementRect(SE_HeaderLabel, option, widget );
           rect.adjust( 0, 1, 0, 1);//poziciata na text na headera
           return rect;
       }
       default:
           return HvPublicStyle::subElementRect (element, option, widget);
       }*/
    ...
    case SE_RadioButtonIndicator:
        rect = visualRect(option->direction, option->rect,
                          HvPublicStyle::subElementRect(element, option, widget)).adjusted(0, 0, 1, 1);
        break;

        //lz2hv implamentations
    case SE_ItemViewItemText:
        rect = HvPublicStyle::subElementRect( SE_ItemViewItemText, option, widget );
        rect.adjust( 0, 1, 0, 1 );//poziciata na text na row
        break;
    case SE_HeaderLabel:
        rect = HvPublicStyle::subElementRect(SE_HeaderLabel, option, widget );
        rect.adjust( 0, 1, 0, 1);//poziciata na text na headera
        break;
        //////////// end hv //////
...
static const int blueFrameWidthsb   =  2;  //2.26 fontcombo box with of line edit focus frame
static const int blueFrameWidthcb =  0;  //2.26 fontcombo box with of line edit focus frame
QRect HvStylePlastique::subControlRect(ComplexControl control, const QStyleOptionComplex *option,
                                       SubControl subControl, const QWidget *widget) const
{
    QRect rect = HvPublicStyle::subControlRect(control, option, subControl, widget);
    ...
    case CC_SpinBox:
        if (const QStyleOptionSpinBox *spinBox = qstyleoption_cast<const QStyleOptionSpinBox *>(option))
        {
            ...
            case SC_SpinBoxEditField:
                ...
                rect.adjust(blueFrameWidthsb, blueFrameWidthsb, -blueFrameWidthsb, -blueFrameWidthsb);
                rect = visualRect(spinBox->direction, spinBox->rect, rect);
                break;
            default:
                break;
    ...
    case CC_ComboBox:
        ...
        case SC_ComboBoxEditField:
        {
            if (const QStyleOptionComboBox *box = qstyleoption_cast<const QStyleOptionComboBox *>(option))
            {
                ...
                if (box->editable)
                {
                    rect = box->rect.adjusted(blueFrameWidthcb, blueFrameWidthcb, -blueFrameWidthcb, -blueFrameWidthcb);
            ...
            case SC_TitleBarLabel:
                if (tb->titleBarFlags & (Qt::WindowTitleHint | Qt::WindowSystemMenuHint))
                {
                    ...
                    if (tb->titleBarFlags & Qt::WindowContextHelpButtonHint)
                        ret.adjust(0, 0, -delta, 0);
                    ret = ret.adjusted(indent, 0, -indent, 0);// 2.26
                }
                break;
...
QSize HvStylePlastique::sizeFromContents(ContentsType type, const QStyleOption * option, const QSize & size, const QWidget * widget ) const
{
    QSize newSize = HvPublicStyle::sizeFromContents(type, option, size, widget);
    int windowsItemHMargin=3;
    int windowsItemVMargin=2;

    switch (type)
    {
    case CT_ItemViewItem://hv
        newSize = HvPublicStyle::sizeFromContents( CT_ItemViewItem, option, size, widget  );
        //size.setHeight(size.height()+1) ;//na gore marda goleminata na row se edno margin QTreeView
        newSize.setWidth(size.width()+4);//otpred marda goleminata na row se edno margin QTreeView
        break;
    case CT_RadioButton:
        ++newSize.rheight();
        ++newSize.rwidth();
    ...
    case CT_ComboBox:
        ...
        newSize.rwidth() += 30; //HV importent -> Make room for drop-down indicator
        newSize.rheight() += 4;
        break;
#endif
//#ifndef QT_NO_MENU
    case CT_MenuItem:
        if (const QStyleOptionMenuItem *menuItem = qstyleoption_cast<const QStyleOptionMenuItem *>(option))
        {
            ...
            if (menuItem->menuItemType != QStyleOptionMenuItem::Separator && !menuItem->icon.isNull())
            {
            ...
            int tabSpacing = 20;
            if (menuItem->text.contains(QLatin1Char('\t'))) w += tabSpacing;
            else if (menuItem->menuItemType == QStyleOptionMenuItem::SubMenu) w += 2 * 2;
...
#if QT_VERSION >= QT_VERSION_CHECK(5, 11, 0) //w += fmBold.width(menuItem->text) - fm.width(menuItem->text);
            ...
            int checkcol = qMax<int>(maxpmw,12); // Windows always shows a check column
            w += checkcol;
            w += 10 + 10;
            newSize.setWidth(w);
            //end 2.69 for Context Menu Items
        }
        break;
//#endif // QT_NO_MENU
//#ifndef QT_NO_MENUBAR
    case CT_MenuBarItem:
        ...
        if (!newSize.isEmpty())
            newSize += QSize(windowsItemHMargin * 4, windowsItemVMargin * 2);
        break;
//#endif
    case CT_PushButton:
        if (const QStyleOptionButton *btn = qstyleoption_cast<const QStyleOptionButton *>(option))
        {
            if (!btn->text.isEmpty() && newSize.width() < 60)//2.33 min width buttons old=80
                newSize.setWidth(60);
                                       ...
                                       const QColor &gradientStop)
{
    ...
    bool doPixmapCache = painter->deviceTransform().isIdentity() && painter->worldTransform().isIdentity();//2.56
...
void HvStylePlastique::drawControl(ControlElement element, const QStyleOption * option, QPainter * painter, const QWidget * widget) const
{

    //painter->setFont(QApplication::font());//lz2hv
    ...
    case CE_TabBarTabShape:
        if (const QStyleOptionTab *tab = qstyleoption_cast<const QStyleOptionTab *>(option))
        {
            ...
            {
                HvPublicStyle::drawControl(element, option, painter, widget);
                break;
            }
    ...
    case CE_ProgressBarLabel:
        if (const QStyleOptionProgressBar *bar = qstyleoption_cast<const QStyleOptionProgressBar *>(option))
        {
            ...
            // Get extra style options if version 2 lz2hv removed
    ...
    case CE_ProgressBarContents:
        if (const QStyleOptionProgressBar *bar = qstyleoption_cast<const QStyleOptionProgressBar *>(option))
        {
            ...
            //stop 2.26 qt5 gcc7.3.1 Linux problem
            /*if (const QStyleOptionProgressBarV2 *bar2 = qstyleoption_cast<const QStyleOptionProgressBarV2 *>(option))
            {
                vertical = (bar2->orientation == Qt::Vertical);
                inverted = bar2->invertedAppearance;
            ...
            QString progressBarName = QStyleHelper1::uniqueName(QLatin1String("progressBarContents"),
                                      option, rect.size());
            QPixmap cache;
            ...
            if (!indeterminate)
            {
                int step = 10;//(AnimateProgressBar || (indeterminate && AnimateBusyProgressBar)) ? (d->animateStep % 20) : 0;
                if (reverse)
    ...
    case CE_HeaderSection:
        ...
        {
            QPixmap cache;
            QString pixmapName = QStyleHelper1::uniqueName(QLatin1String("headersection"), option, option->rect.size());
    ...
    case CE_MenuBarItem:
        ...
        {
            QPixmap cache;
            QString pixmapName = QStyleHelper1::uniqueName(QLatin1String("menubaritem"), option, option->rect.size());
            if (!QPixmapCache::find(pixmapName, &cache))
            {
                cache = QPixmap(option->rect.size());
                cache.fill(Qt::white);
        ...
        if (const QStyleOptionMenuItem *mbi = qstyleoption_cast<const QStyleOptionMenuItem *>(option))
        {
            ...
            //QFont fff("arial",11);
            // QFont fff = mbi->font;
            //qDebug()<<"FFFFF="<<fff.family();
            //painter->setFont(QApplication::font());
            //painter->font().setFamily("arial");
            //qDebug()<<"CE_MenuBarItem="<<painter->font().family();

            HvPublicStyle::drawControl(element, &newMI, painter, widget);
        }
        break;
    ...
    case CE_DockWidgetTitle:
        if (const QStyleOptionDockWidget *dockWidget = qstyleoption_cast<const QStyleOptionDockWidget *>(option))
        {
            ...
            //const QStyleOptionDockWidgetV2 *v2 lz2hv removed
            //= qstyleoption_cast<const QStyleOptionDockWidgetV2*>(dockWidget);
            //bool verticalTitleBar = v2 == 0 ? false : v2->verticalTitleBar;
            bool verticalTitleBar = false; // lz2hv added
    ...
    case CE_ScrollBarAddLine:
        if (const QStyleOptionSlider *scrollBar = qstyleoption_cast<const QStyleOptionSlider *>(option))
        {
            ...
            QString addLinePixmapName = QStyleHelper1::uniqueName(QLatin1String("scrollbar_addline"), option, option->rect.size());
            QPixmap cache;
    ...
    case CE_ScrollBarAddPage:
        if (const QStyleOptionSlider *scrollBar = qstyleoption_cast<const QStyleOptionSlider *>(option))
        {
            ...
            QString groovePixmapName = QStyleHelper1::uniqueName(QLatin1String("scrollbar_groove"), option, option->rect.size());
            if (sunken)
    ...
    case CE_ScrollBarSubLine:
        if (const QStyleOptionSlider *scrollBar = qstyleoption_cast<const QStyleOptionSlider *>(option))
        {
            ...
            QString subLinePixmapName = QStyleHelper1::uniqueName(QLatin1String("scrollbar_subline"), option, button1.size());
            QPixmap cache;
    ...
    case CE_ScrollBarSlider:
        if (const QStyleOptionSlider *scrollBar = qstyleoption_cast<const QStyleOptionSlider *>(option))
        {
            ...
            {
                QString sliderPixmapName = QStyleHelper1::uniqueName(QLatin1String("scrollbar_slider"), option, option->rect.size());
                if (horizontal)
...
void HvStylePlastique::drawPrimitive(PrimitiveElement element, const QStyleOption *option, QPainter *painter, const QWidget *widget) const
{
    /*if (element == HvPublicStyle::PE_IndicatorBranch)// || QStyle::State_Open|| QStyle::PE_Frame
    {
        painter->save();
        painter->setPen(QPen(QColor(255,207,0), 1, Qt::SolidLine));//vazno da e Qt::SolidLine ina4e nakasva liniata pri lazli4ni razmeri Qt::white, 1, Qt::DashLine
        HvPublicStyle::drawPrimitive(element, option, painter, widget);
        painter->restore();
    }
    HvPublicStyle::drawPrimitive(element, option, painter, widget);
    return;*/
    ...
    case PE_FrameTabBarBase:
        ...
        break ;
#endif // QT_NO_TABBAR
#ifndef QT_NO_GROUPBOX //2.26 lz2hv qt5 on
...
#endif
        QString pixmapName = QStyleHelper1::uniqueName(QLatin1String("toolbarhandle"), option, rect.size());
        if (!QPixmapCache::find(pixmapName, &cache))
        {
            cache = QPixmap(rect.size());
            cache.fill(Qt::transparent);
    ...
    case PE_IndicatorDockWidgetResizeHandle:
        ...
        break;
    case PE_FrameFocusRect: // punktira na butonite hv
        if (const QStyleOptionFocusRect *fropt = qstyleoption_cast<const QStyleOptionFocusRect *>(option))
        {
            ...
            QColor patternCol((bg_col.red() ^ 0xff) & 0xff,
                              ...
                              (bg_col.blue() ^ 0xff) & 0xff,127);// , 127 <- prozra4nost
            painter->setBrush(QBrush(patternCol, Qt::Dense4Pattern));
            painter->setBrushOrigin(r.topLeft());
            painter->setPen(Qt::NoPen);
            //lz2hv stop button rect ......
...
void HvStylePlastique::drawComplexControl(ComplexControl control, const QStyleOptionComplex *option,
        QPainter *painter, const QWidget *widget) const
{
    ...
    case CC_Slider:
        if (const QStyleOptionSlider *slider = qstyleoption_cast<const QStyleOptionSlider *>(option))
        {
            ...
            {
                QString handlePixmapName = QStyleHelper1::uniqueName(QLatin1String("slider_handle"), option, handle.size());
                ...
                if (!QPixmapCache::find(handlePixmapName, &cache))
                {
                    ...
                    handlePainter.fillRect(pixmapRect, option->palette.window());
    ...
    case CC_SpinBox:
        if (const QStyleOptionSpinBox *spinBox = qstyleoption_cast<const QStyleOptionSpinBox *>(option))
        {
            ...
            //Q_D(const HvStylePlastique);
            //if (spinBox->buttonSymbols == QAbstractSpinBox::NoButtons)
            {
                //hv org v1.35 QRect filledRect = option->rect.adjusted(1, 1, -1, -1);
                QRect filledRect = option->rect.adjusted(0, 0, -0, -0);
    ...
    case CC_TitleBar:
        if (const QStyleOptionTitleBar *titleBar = qstyleoption_cast<const QStyleOptionTitleBar *>(option))
        {
...

            {
                ...
                bool tooWide = (QFontMetrics(font).horizontalAdvance(titleBar->text) > textRect.width());//2.56
#else
                bool tooWide = (QFontMetrics(font).width(titleBar->text) > textRect.width());//2.56
#endif
    ...
    case CC_Dial:
        if (const QStyleOptionSlider *dial = qstyleoption_cast<const QStyleOptionSlider *>(option))
            QStyleHelper1::drawDial(dial, painter);
        break;
#endif // QT_NO_DIAL
    default:
        HvPublicStyle::drawComplexControl(control, option, painter, widget);
        break;
    }
}

void HvStylePlastique::polish(QWidget *widget)
{
...
/*!
  \reimp
*/
void HvStylePlastique::unpolish(QWidget *widget)
{
    if (qobject_cast<QPushButton *>(widget)
#ifndef QT_NO_COMBOBOX
            || qobject_cast<QComboBox *>(widget)
...
/*!
  \reimp
*/
void HvStylePlastique::polish(QApplication *app)
{
    HvPublicStyle::polish(app);
}
...
*/
/*
QPainterPath HvStylePlastique::roundRectPath(const QRect &rect)
{
...
/*
void HvStylePlastique::polish(QPalette & palette)
{
	QColor brown(212, 212, 212);//(212, 140, 95);
    QColor beige(250, 250, 250);//(236, 182, 120);
    QColor backg(240, 240, 240);
    QColor slightlyOpaqueBlack(0, 0, 0, 63);

    //QPixmap backgroundImage(":/images/woodbackground.png");
    //QPixmap backgroundImage(":font/woodbutton.png");
    QPixmap buttonImage(":font/woodbutton.png");
...
void HvStylePlastique::setTexture(QPalette &palette, QPalette::ColorRole role,
...
*/
/*void HvStylePlastique::SetStyleHV(bool f)
{
    f_dark_style = f;
}*/
void HvStylePlastique::polish(QPalette & pal)
{
    if (f_dark_style)
    {
        pal.setColor(QPalette::Button, QColor(70,70,70)); // header color
        pal.setColor(QPalette::ButtonText, QColor(255,255,255));
        //pal.setColor(QPalette::BrightText, QColor(50,100,160)); // kontura na lista butonite
        pal.setColor(QPalette::Window, QColor(90,90,90)); // kantovete na prozoreca
        pal.setColor(QPalette::Foreground, QColor(255,255,255)); // strelkite na scrola width
        pal.setColor(QPalette::BrightText, QColor(0,150,0)); // ne znam ama e zeleno
        pal.setColor(QPalette::Base, QColor(25,25,25));// backgrownd
        pal.setColor(QPalette::Text, QColor(255,255,255));// bukwi
        pal.setColor(QPalette::HighlightedText, QColor(255,255,255));//selected Text color
        pal.setColor(QPalette::Highlight, QColor(80,80,80)); // selected row
        pal.setColor(QPalette::Light, QColor(110,110,110));// zaradi sort indicatora
        pal.setColor(QPalette::Midlight, QColor(65,65,65));// zaradi sort indicatora
        pal.setColor(QPalette::Dark, QColor(30,30,30));// zaradi sort indicatora
        pal.setColor(QPalette::Mid, QColor(20,20,20));// zaradi sort indicatora
        pal.setColor(QPalette::Shadow, QColor(0,0,0));// zaradi bordera okantovaneto na lista
        //pal.setColor(QPalette::ToolTipBase, QColor(255,255,255));// bukwi

        QBrush brush = pal.window();
        //brush.setColor(brush.color().dark());
        brush.setColor(brush.color().darker());//2.56
        pal.setBrush(QPalette::Disabled, QPalette::WindowText, QColor(150,150,150));
        pal.setBrush(QPalette::Disabled, QPalette::Text, QColor(125,125,125));
        pal.setBrush(QPalette::Disabled, QPalette::ButtonText, QColor(100,100,100));
        pal.setBrush(QPalette::Disabled, QPalette::Base, brush);
        pal.setBrush(QPalette::Disabled, QPalette::Button, QColor(50,50,50));
...
/*!
  \reimp
*/
void HvStylePlastique::unpolish(QApplication *app)
{
    HvPublicStyle::unpolish(app);
}
/*
QIcon HvStylePlastique::standardIconImplementation(StandardPixmap standardIcon, const QStyleOption *option,
                                                  const QWidget *widget) const
{
    return HvPublicStyle::standardIconImplementation(standardIcon, option, widget);
}
*/
QPixmap HvStylePlastique::standardPixmap(StandardPixmap standardPixmap, const QStyleOption *opt,
        const QWidget *widget) const
{
    return HvPublicStyle::standardPixmap(standardPixmap, opt, widget);
}
...
//old qt4 protected slot
//int HvStylePlastique::layoutSpacingImplementation(QSizePolicy::ControlType control1,
//QSizePolicy::ControlType control2,
//Qt::Orientation orientation,
//const QStyleOption * /* option */,
//const QWidget * /* widget */) const
//new qt5  old is qt4 layoutSpacingImplementation
int HvStylePlastique::layoutSpacing(QSizePolicy::ControlType control1,
                                    QSizePolicy::ControlType control2,
                                    Qt::Orientation orientation,
                                    const QStyleOption * /* option */,
                                    const QWidget * /* widget */) const
...
/*
QPalette HvStylePlastique::standardPalette() const
{
    QPalette palette;
...
int HvStylePlastique::styleHint(StyleHint hint, const QStyleOption * opt, const QWidget * widget, QStyleHintReturn * returnData) const
{
    /*int ret = 0;
    switch (hint) {
    ...
    #endif
    default:
        ret = HvPublicStyle::styleHint(hint, option, widget, returnData);
        break;
    }
    return ret;*/
    int ret = 0;
...
QStyle::SubControl HvStylePlastique::hitTestComplexControl(ComplexControl control, const QStyleOptionComplex *option,
        const QPoint &pos, const QWidget *widget) const
{
    ...
    return ret != SC_None ? ret : HvPublicStyle::hitTestComplexControl(control, option, pos, widget);
}

/*
bool HvStylePlastique::eventFilter(QObject *watched, QEvent *event)
{
    ...
    return HvPublicStyle::eventFilter(watched, event);
}

void HvStylePlastique::timerEvent(QTimerEvent *event)
{
#ifndef QT_NO_PROGRESSBAR
    Q_D(QPlastiqueStyle);
